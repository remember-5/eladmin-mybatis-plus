...
public class QuartzManage {
     ...
     * @param quartzJob /
     */
    public void updateJobCron(QuartzJobModel quartzJob) {
        try {
            ...
            trigger.getJobDataMap().put(QuartzJobModel.JOB_KEY, quartzJob);
     ...
     * @param quartzJob /
     */
    public void deleteJob(QuartzJobModel quartzJob) {
        try {
     ...
     * @param quartzJob /
     */
    public void runJobNow(QuartzJobModel quartzJob) {
        try {
            TriggerKey triggerKey = TriggerKey.triggerKey(JOB_NAME + quartzJob.getId());
            CronTrigger trigger = (CronTrigger) scheduler.getTrigger(triggerKey);
            // 如果不存在则创建一个定时任务
            ...
            dataMap.put(QuartzJobModel.JOB_KEY, quartzJob);
            JobKey jobKey = JobKey.jobKey(JOB_NAME + quartzJob.getId());
