...
import com.admin.tools.mapper.IAlipayConfigMapper;
import com.admin.tools.model.AlipayConfigModel;
import com.admin.tools.service.IAliPayService;
...
public class AdminAlipayServiceImpl extends ServiceImpl<IAlipayConfigMapper, AlipayConfigModel>
        implements IAliPayService {
    @Override
    @Cacheable(key = "'config'")
    public AlipayConfigModel find() {
        return Optional.ofNullable(this.getById(1L)).orElseGet(AlipayConfigModel::new);
    }

    @Override
    @CachePut(key = "'config'")
    @Transactional(rollbackFor = Exception.class)
    public AlipayConfigModel config(AlipayConfigModel alipayConfig) {
    ...
    @Override
    public String toPayAsPc(AlipayConfigModel alipay, TradeVo trade) throws Exception {
    ...
    @Override
    public String toPayAsWeb(AlipayConfigModel alipay, TradeVo trade) throws Exception {
        if (alipay.getId() == null) {
            throw new BadRequestException("请先添加相应配置，再操作");
        }
        AlipayClient alipayClient = new DefaultAlipayClient(alipay.getGatewayUrl(), alipay.getAppId(),
