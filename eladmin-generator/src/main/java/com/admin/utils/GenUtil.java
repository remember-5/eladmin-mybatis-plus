...
public class GenUtil {
    ...
    public static List<Map<String, Object>> preview(List<ColumnInfoModel> columns, GenConfigModel genConfig) {
    ...
    public static String download(List<ColumnInfoModel> columns, GenConfigModel genConfig) throws IOException {
        // 拼接的路径：/tmpeladmin-gen-temp/，这个路径在Linux下需要root用户才有权限创建,非root用户会权限错误而失败，更改为：
        // /tmp/eladmin-gen-temp/
        // String tempPath =System.getProperty("java.io.tmpdir") + "eladmin-gen-temp" +
        // File.separator + genConfig.getTableName() + File.separator;
        String tempPath = System.getProperty("java.io.tmpdir") + File.separator + "eladmin-gen-temp" + File.separator
    ...
    public static void generatorCode(List<ColumnInfoModel> columnInfos, GenConfigModel genConfig) throws IOException {
    ...
    // 获取模版数据
    private static Map<String, Object> getGenMap(List<ColumnInfoModel> columnInfos, GenConfigModel genConfig) {
        // 存储模版字段数据
        ...
        for (ColumnInfoModel column : columnInfos) {
     ...
     * 定义后端文件路径以及名称
     */
    private static String getAdminFilePath(String templateName, GenConfigModel genConfig, String className,
                                           String rootPath) {
